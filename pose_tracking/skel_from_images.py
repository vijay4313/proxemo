...
if __name__ == "__main__":
    ...
    display = True
    # init
    pipeline, csonfig = init_realsense_capture()
    sdk_path = init_skel_track(verbose)
    api = init_cubemos_api(sdk_path)
    image_files = glob.glob('/home/emotiongroup/Desktop/rosbag_img/bala_sad_with/*')
    # fourcc = cv2.VideoWriter_fourcc(*'mp4v') 
    videoObj = cv2.VideoWriter('video_without_sad.mp4',-1, 1,(640,480))
    image_files.sort()
    print(len(image_files))
    for ndx, image_file in enumerate(image_files):
        print(ndx)
        color_image = cv2.imread(image_file)
        color_image_org = copy.deepcopy(color_image)
        # cv2.imshow("rest", color_image)
        # capture
        ...
        if display:
            ...
            if len(skeletons) > 0:
                out_image = np.hstack((color_image_org, skel_img))
                out_image = skel_img
                videoObj.write(out_image)
            else:
                out_image = color_image
                # videoObj.write(color_image)
                # images = np.hstack((images, color_image))
            # Show images
            cv2.imwrite(f"./bala_sad_with/{ndx}.jpg", out_image)
