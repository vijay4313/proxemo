...
class Real_Sense_Camera():
    def __init__(self, clipping_distance_meters, kernel_size):
        ...
        self.avg_kernel = np.ones((kernel_size, kernel_size), np.float32)/(kernel_size**2)
    ...
    def capture(self):
        while True:
            ...
            self.depth_image_align = np.asanyarray(aligned_depth_frame.get_data())
            ...
            depth_image_3d = np.dstack((self.depth_image_align,
                                        ...
                                        self.depth_image_align))
            depth_mask = (depth_image_3d > self.clipping_distance) | (depth_image_3d <= 0)
            self.bg_removed = np.where(depth_mask, grey_color, self.color_image)
